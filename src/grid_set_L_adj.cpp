...
Point2d<double> grid::closest_corner(const label& L, const Point2d<double>& V)
{
	...
	switch (d)
	{
	case 1: return Point2d<double>(x.at(L(0)),y.at(L(1)));
	case 2: return Point2d<double>(x.at(L(0)+1),y.at(L(1)));
	case 3: return Point2d<double>(x.at(L(0)+1),y.at(L(1)+1));
	case 4: return Point2d<double>(x.at(L(0)),y.at(L(1)+1));
	}
