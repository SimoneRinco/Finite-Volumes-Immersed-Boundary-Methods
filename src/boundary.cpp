...
boundary::boundary(const boundary& lhs)
  : vertices_and_labels(lhs.vertices_and_labels)
  , N_edges(lhs.N_edges)
{}

Point2d<double> boundary::get_edge_vertex(const BOUNDARY::side& s, unsigned int n_edge) const
{
  auto it=vertices_and_labels.begin();

  if (N_edges==0)
  {
    throw ERRORS::errors(1);
  }
...
void boundary::set_edge_vertex(
  const BOUNDARY::side& s,
  const Point2d<double>& vertex,
  unsigned int n_edge)
{
  auto it=vertices_and_labels.begin();
...
{
  // aggiunge un nuovo lato con l'etichetta l inserendo un nuovo vertice in coda
  // al vettore dei vertici.
  // Di conseguenza viene modificato il vertice sinistro del primo lato.
  // Gli altri lati rimangono inalterati.

  using VertexLabel = std::pair<Point2d<double>, IMMERSED_BOUNDARY::label>;
  if (N_edges==0)
  {
    vertices_and_labels.push_back(VertexLabel(last_vertex,l));
  }
  else
  {
    IMMERSED_BOUNDARY::label first_label=vertices_and_labels.rbegin()->second;
    vertices_and_labels.rbegin()->second = l;
    vertices_and_labels.push_back(VertexLabel(last_vertex,first_label));
  }
  N_edges++;

}

void boundary::add_edge(
